...
referenced_sheet="$script_dir/../referenced_sheets/commit_nv.csv"
...
while IFS=, read -r ID project_name module_name url sha commit_n_25 commit_n_50 commit_n_75 commit_n_100; do
    if [ "$ID" == "$module_id" ]; then
        module_path="$project_path/$module_name"
        sha_to_checkout=$commit_n_100
        module=$module_name
        break
    fi
...
fix_project_for_init_run() {
    ...
    if [[ "$project_name" == "openpojo" ]]; then
        ...
        # sed -i '188s~5.2.2-SNAPSHOT~5.2.3~g' pom.xml
        inject_plugin_management pom.xml
        # sed -i 's~5.2.2-SNAPSHOT~5.2.3~g' pom.xml
        # cd $module_path
        # sed -i 's~5.2.2-SNAPSHOT~5.2.3~g' pom.xml
        # cd -
        # in line 188 change the version to 5.2.3
