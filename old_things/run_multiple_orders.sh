...
create_mvn_instance() {
    ...
    # print the JAVA_HOME environment variable
    echo $JAVA_HOME
...
setup_custom_surefire_on_mvn_instance() {
    ...
    cp surefire-changing-maven-extension/target/surefire-changing-maven-extension-1.0-SNAPSHOT.jar $mvn_instance_base_dir/$module_id/maven/lib/ext/
    cd -
}
...
fix_project_for_init_run() {
    ...
    # elif [[ $project_name == "dropwizard" ]]; then
    #     MVNINSTALLOPTIONS="-Ddependency-check.skip=true"
    
    # elif [[ $project_name == "riptide" ]]; then
    #     MVNINSTALLOPTIONS="-Ddependency-check.skip=true"
...
while true; do
    ...
    if [ $done_order -eq $num_of_orders ]; then
        break
    fi
